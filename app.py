...
def position_size(score, flags):
    ...
    mult = 1.0
    for f in flags:
        mult *= {"High ATR":0.7,"Gap":0.75,"Extended":0.8}.get(f,1)
...
def send_to_discord(df):
    ...
    payload = {
        "content": "🚀 **Swing Scanner — Résultats**\n\n" +
                   "\n".join(lines[:TOP_N])
    }
...
def scan_universe(tickers):
    ...
    for t in tickers:
        ...
        total = round(
            W_S1*s1 + W_S2*s2 + W_S3*s3 + W_S4*s4, 2
        )
...
# UI
# =====================================================
st.title("📊 Swing Scanner — Envoi Discord")
